...
public class LocationTrackUtil {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(LocationTrackUtil.class);
	...
	public static Pair getApiDtl(String postalCode) throws IOException, SystemException {
		Pair p1 = new Pair();

		try (CloseableHttpClient httpclient = HttpClients.createDefault()) {
			HttpGet httpget = new HttpGet(
					String.format(SysPropertiesHelper.getProperty("GEOCODERAPI_URL"), postalCode));
			httpget.setHeader(SysPropertiesHelper.getProperty("HEADER_TOKEN_NAME"),
					SysPropertiesHelper.getProperty("HEADER_TOKEN_VALUE"));
			try (CloseableHttpResponse response = httpclient.execute(httpget)) {
				...
				if (response.getStatusLine().getStatusCode() != 200) {
					// System.out.println("Connection is bad, status code: " +
					// response.getStatusLine().getStatusCode());
					LOGGER.error("Connection is bad while trying to connect geocoder API, status code: {}.",
							response.getStatusLine().getStatusCode());
				} else {
	...
	public static Pair getLocationFromPostalCode(String postalCode) {
		...
		try {
			...
			if (!isRecFoundFlg) {
				...
				LOGGER.info(
						"Postal-code: {} information does not exist in input file, so trying to fetch corresponding latitude: {} and longitude: {} from geocoder API.",
						postalCode, p.getLatitude(), p.getLongitude());
			}

		} catch (IOException | NumberFormatException | SystemException e) {
			// System.out.println("Error-" + e.getMessage()); // don't do any restricted
			// activities, only log/show messaging
	...
	public static List<Pair> getNearestLocations(Pair sourceLocation, List<Pair> listOfTargetLocations) {
		...
		if (!(sourceLocation.getLatitude() == 0 && sourceLocation.getLongitude() == 0)) {
			try {
				...
				for (Pair p : listOfTargetLocations) {
					...
					pd.setDistance(distance(sourceLocation.getLatitude(), sourceLocation.getLongitude(),
